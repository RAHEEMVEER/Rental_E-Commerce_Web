...
export default function Header({ login }: { login: React.ReactNode }) {
  return (
    <header className="flex flex-col gap-3 sticky top-0 z-50 w-full bg-white py-3 px-4 md:flex-row md:px-8 justify-between items-center min-h-[80px] shadow-md box-border">
      <div className="flex justify-between items-center w-full md:max-w-max"><Link href="/" className="text-[#3563E9] text-[28px] font-semibold">MORENT</Link><div className="w-9 h-9 md:hidden rounded-full">{login}</div></div>
      ...
      <div className="hidden md:flex gap-4 items-center">
        ...
        <div className="w-9 h-9 rounded-full">{login}</div>
      </div>
    </header>
  );
}